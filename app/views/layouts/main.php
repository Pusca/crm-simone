...
<?php if ($user): ?>
    <div class="min-h-screen lg:flex">
        <div class="no-print fixed inset-0 z-40 hidden bg-slate-950/50 lg:hidden" data-sidebar-overlay></div>

        <aside class="no-print fixed inset-y-0 left-0 z-50 flex w-72 max-w-[85vw] -translate-x-full flex-col border-r border-slate-700 bg-slate-900 text-slate-100 transition-transform duration-200 ease-out lg:static lg:w-72 lg:max-w-none lg:translate-x-0 lg:border-b-0" data-sidebar>
            ...
            <nav class="px-4 pb-6">
                ...
                <?php if ($user['role'] === 'manager'): ?>
                    <div class="mt-5 border-t border-slate-700 pt-4">
                        ...
                        <a class="sidebar-link <?= is_active_path('/settings/pipeline') ? 'sidebar-link-active' : '' ?>" href="<?= e(base_url('settings/pipeline')) ?>" data-sidebar-link>Pipeline</a>
                        <a class="sidebar-link <?= is_active_path('/settings/categories') ? 'sidebar-link-active' : '' ?>" href="<?= e(base_url('settings/categories')) ?>" data-sidebar-link>Categorie Preventivi</a>
                        <a class="sidebar-link <?= is_active_path('/settings/targets') ? 'sidebar-link-active' : '' ?>" href="<?= e(base_url('settings/targets')) ?>" data-sidebar-link>Target</a>
        ...
        <main class="flex-1 p-4 lg:p-8">
            <div class="no-print mb-4 flex items-center justify-between gap-3 lg:hidden">
            ...
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="no-print flex flex-wrap items-center gap-3">
                    <button type="button" class="rounded border border-slate-300 px-3 py-2 text-sm hover:bg-slate-50" onclick="window.print()">Stampa pagina</button>
            ...
            <?php if ($error = flash('error')): ?>
                <div class="no-print mb-4 rounded border border-red-200 bg-red-50 px-4 py-3 text-red-700"><?= e($error) ?></div>
            <?php endif; ?>
            <?php if ($success = flash('success')): ?>
                <div class="no-print mb-4 rounded border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-700"><?= e($success) ?></div>
            <?php endif; ?>
